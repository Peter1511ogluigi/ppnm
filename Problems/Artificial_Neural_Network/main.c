...
int main(){
	printf("- - - - Excercise A - - - - \n\n");
	printf("We construct a simple ann with 3 neurons and an activation function of f(x) = sin(x)*exp(-x)  where the acquired parameters become:\n\n");
	int nx = 20;
...
printf("\n\n");
printf("- - - - Exercise B - - - - \n\n");
printf("We are to modify such that both the derivative f'(x)= exp(-x)*(sin(x)-cos(x))\n");
printf("and the antiderivative F(x)=-1/2*exp(-x)(cos(x)+sin(x))+1/2*exp(-x0)(cos(x0)+sin(x0))\n");
printf("can be approximated after the training. \n The solution can be seen on plots.png.");
