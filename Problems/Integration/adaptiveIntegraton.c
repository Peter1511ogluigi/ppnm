...
static double FF(double f(double),double t){// variable transformation formula
	return f( A+(1-t)/t )/t/t;
	}


double wrap24( double f(double),double a, double b,
double acc, double eps, double f2, double f3, int nrec){//wrapper calls F(f,t)
	assert(nrec<99);
	double f1=FF(f,a+(b-a)/6), f4=FF(f,a+5*(b-a)/6);
...
double integ_a_inf(double f(double),double a,double acc,double eps ){
	A=a; // save left limit in file-scope variable
	a=0; double b=1;
	double f2=FF(f,a+2*(b-a)/6),f3=FF(f,a+4*(b-a)/6);
	int nrec=0;
